...
import numpy as np
from voice_stream import compiled_cache_path, gate_report


def load(path,cache,gate):
    target = compiled_cache_path(path,cache)
    cache.mkdir(parents=True,exist_ok=True)
    if not target.exists():
        ct.models.utils.compile_model(str(path),destination_path=str(target))
    result = subprocess.run([str(gate),str(target)],capture_output=True,text=True)
    if result.returncode:
        raise RuntimeError(f'ANE admission failed for {path}: {result.stderr.strip() or result.stdout.strip()}')
    admission = gate_report(result.stdout)
    return ct.models.CompiledMLModel(str(target),compute_units=ct.ComputeUnit.CPU_AND_NE),admission
...
def main():
    ...
    candidate,admission = load(args.candidate,args.cache,args.gate)
