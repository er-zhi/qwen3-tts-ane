# Qwen3-TTS 0.6B on Apple Neural Engine — Experiment Log

> Historical record: this file intentionally preserves intermediate measurements,
> rejected candidates and statements that were true only at that stage. Use the
> root `README.md`, `MODEL_CARD.md` and `BENCHMARKS.json` for current release
> claims. Do not copy an isolated number from this log without its scope.
