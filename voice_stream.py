...
def cached_file_digest(path):
    state = path.stat()
    identity = (state.st_dev, state.st_ino, state.st_size, state.st_mtime_ns)
    value = _FILE_DIGEST_CACHE.get(identity)
    if value is None:
        with path.open("rb") as source:
            value = hashlib.file_digest(source, "sha256").digest()
...
def gate_report(stdout):
    ...
    if (
        ...
        or report.get("ane_operation_ratio") != 1
        or report.get("cpu_preferred_operations") != 0
        or report.get("gpu_preferred_operations") != 0
    ):
...
def compiled_cache_path(package, directory):
    ...
    signature = [
        (
            str(path.relative_to(package)) if package.is_dir() else path.name,
            path.stat().st_size,
            path.stat().st_mtime_ns,
        )
        for path in files
    ]
    memo_dir = directory / ".source-hashes"
    ...
    for path, (relative, _, _) in zip(files, signature, strict=True):
        digest.update(relative.encode())
        digest.update(cached_file_digest(path))
...
def admit_ane_package(package, gate, directory=None, fast_prediction=False):
    def inspect(target):
        ...
        checked = subprocess.run(command, capture_output=True, text=True)  # noqa: S603
    ...
    if gate is None:
        if directory is None:
            raise ValueError("A compiled model directory is required without an ANE gate")
        ...
        return {"status": "COMPILED", "compiled_model": str(target), "ane_admission_checked": False}
    if directory is None:
    ...
    with target.with_suffix(".lock").open("a") as lock:
        ...
        if existing and identity is not None and marker.exists():
            try:
                ...
                if (
                    ...
                    and Path(report["compiled_model"]).resolve() == target.resolve()
                ):
        ...
        try:
            ...
            if not existing or rejected.exists():
                raise RuntimeError(
                    f"ANE admission failed; no further automatic rebuild for {target}: {error}"
                ) from error
            target.rename(rejected)
            print(
                f"Rebuilding rejected compiled artifact; preserved at {rejected}: {error}",
                flush=True,
            )
            ct.models.utils.compile_model(str(package), destination_path=str(target))
            report = inspect(target)
        ...
        if identity is not None:
            ...
            temporary.write_text(
                json.dumps({"identity": identity, "report": report}, indent=2) + "\n"
            )
...
def capture_prompt(tts, text, instruction, speaker, capacity):
    ...
    try:
        if tts.model.tts_model_type == "custom_voice":
            ...
            tts.generate_custom_voice(
                text=text, language="English", speaker=speaker, non_streaming_mode=False
            )
        else:
            tts.generate_voice_design(
                text=text, language="English", instruct=instruction, non_streaming_mode=False
            )
    except StopCapture:
        pass
    finally:
    ...
    arrays = {
        name: captured[name].detach().numpy().copy()
        for name in ["inputs_embeds", "trailing_text_hidden", "tts_pad_embed"]
    }
...
def prepare(
    ...
    if frontend_assets is not None:
        ...
        return prepare_runtime_assets(
            frontend_assets, text, speaker, text_projection, frontend_sink
        )
    import torch
    ...
    embeddings += [
        layer.weight.detach().numpy().copy()
        for layer in talker.code_predictor.get_input_embeddings()
    ]
    ...
    with torch.inference_mode():
        ...
        control_groups = []
        for model, control_capacity, multimodal in [
            (talker.model, capacity, True),
            (talker.code_predictor.model, 16, False),
        ]:
            controls = []
            for pos in range(control_capacity):
                ...
                controls.append(
                    dict(
                        write_mask=write,
                        attention_mask=mask,
                        cosine=cos.unsqueeze(1).numpy().copy(),
                        sine=sin.unsqueeze(1).numpy().copy(),
                    )
                )
...
class VoiceStream:
    def __init__(
        ...
        instruction,
        compiled_dir=None,
        predictor_package=None,
        prefill_packages=None,
        first_decoder_package=None,
        speaker=None,
        model_prefix="qwen17",
        block_count=4,
        decoder_package=None,
        tail_decoder_package=None,
        decoder_windows=None,
        startup_decoder_package=None,
        experimental_history_decoder=False,
        experimental_prefix_state=False,
        short_talker_package=None,
        text_projection_package=None,
        ...
        if experimental_prefix_state and (
            prefill_packages is not None or model_prefix != "qwen06" or not speaker
        ):
            raise ValueError(
                "Prefix-state experiment requires Qwen06 CustomVoice without batched prefill"
            )
        if experimental_history_decoder and (
            decoder_package is None
            or any(
                value is not None
                for value in [
                    first_decoder_package,
                    tail_decoder_package,
                    decoder_windows,
            ...
            raise ValueError(
                "Explicit-history experiment requires only decoder-package, without window decoders"
            )
        self.history_decoder = experimental_history_decoder

        def load(name):
            path = packages / name
            print(f"{'Checking ANE' if gate else 'Loading Core ML'}: {name}", flush=True)
            report = admit_ane_package(path, gate, compiled_dir)
            compiled_path = report["compiled_model"]
            model = ct.models.CompiledMLModel(compiled_path, compute_units=compute_units)
            model.source_spec = ct.utils.load_spec(str(path))
            print(f"Loaded Core ML: {name}", flush=True)
            return model
        ...
        if text_projection_package is not None:
            ...
            inputs = {
                x.name: list(x.type.multiArrayType.shape)
                for x in projection.source_spec.description.input
            }
            outputs = {
                x.name: list(x.type.multiArrayType.shape)
                for x in projection.source_spec.description.output
            }
            ...
            if frontend_assets is not None:
                ...
                self.text_projection = NumpyANETextProjection(
                    projection, shape[1], shape[3], out[1]
                )
            else:
        ...
        gc.collect()
        self.blocks = [
            load(f"{model_prefix}_cached_block{i}.mlpackage") for i in range(block_count)
        ]
        self.long_blocks = (
            [load(str(long_talker_package.resolve()))] if long_talker_package else None
        )
        if self.long_blocks:
            ...
            if (
                not description.state
                or not self.blocks[0].source_spec.description.state
                or masks.get("attention_mask") != [1, 1, 1, 512]
                or masks.get("write_mask") != [1, 1, 512, 1]
            ):
                raise ValueError("Long talker requires stateful 128-to-512 cache migration")
        self.short_blocks = (
            [load(str(short_talker_package.resolve()))] if short_talker_package else None
        )
        if self.short_blocks:
            ...
            if (
                not spec.state
                or masks.get("attention_mask") != [1, 1, 1, 16]
                or masks.get("write_mask") != [1, 1, 16, 1]
            ):
                raise ValueError("Short talker must have stateful 16-position masks")
        self.predictor = load(
            str(predictor_package.resolve()) if predictor_package else "qwen17_predictor.mlpackage"
        )
        self.stateful_predictor = bool(self.predictor.source_spec.description.state)
        self.fused_predictor = {x.name for x in self.predictor.source_spec.description.input} == {
            "past_hidden",
            "first_embedding",
        }
        ...
        self.decoder = load(
            str(decoder_package.resolve()) if decoder_package else "qwen17_decoder32.mlpackage"
        )
        ...
        if self.history_decoder:
            ...
            self.decoder_state_shapes = {
                ...
                if item.name.startswith("in_")
            }
            names = {item.name for item in description.input}
            outputs = {
                item.name: tuple(item.type.multiArrayType.shape) for item in description.output
            }
            if (
                not self.decoder_state_shapes
                or names
                != set(self.decoder_state_shapes) | {"latent", "cosine", "sine", "attention_mask"}
                or outputs.get("pcm") != (1, 1920)
                or any(
                    outputs.get("out_" + name[3:]) != shape
                    for name, shape in self.decoder_state_shapes.items()
                )
            ):
                raise ValueError("Invalid explicit-history decoder contract")
        self.first_decoder = (
            load(str(first_decoder_package.resolve())) if first_decoder_package else None
        )
        self.first_decoder_frames = (
            int(
                next(
                    x
                    for x in self.first_decoder.source_spec.description.input
                    if x.name == "latent"
                ).type.multiArrayType.shape[-1]
            )
            if self.first_decoder
            else 0
        )
        self.tail_decoder = (
            load(str(tail_decoder_package.resolve())) if tail_decoder_package else None
        )
        self.startup_decoder = (
            load(str(startup_decoder_package.resolve())) if startup_decoder_package else None
        )
        ...
        if decoder_windows:
            ...
            for width in range(8, 33, 4):
                self.decoder_windows[width] = load(
                    str((decoder_windows / f"decoder{width}.mlpackage").resolve())
                )
        self.prefill_blocks = (
            [
                load(
                    str((prefill_packages / f"{model_prefix}_prefill_block{i}.mlpackage").resolve())
                )
        ...
        if experimental_prefix_state:
            ...
            if prefix.shape[1] != 10 or not all(
                block.source_spec.description.state for block in self.blocks
            ):
                raise ValueError(
                    "Prefix-state experiment requires the verified 9+1 prompt and stateful blocks"
                )
            ...
            for position in range(9):
                ...
                for block, state in zip(prefix_blocks, self.prefix_states, strict=True):
                    inputs = dict(self.prepared[3][0][position], embeddings=hidden)
                    cache_capacity = next(
                        item.type.multiArrayType.shape[2]
                        for item in block.source_spec.description.input
                        if item.name == "write_mask"
                    )
                    if inputs["write_mask"].shape[2] != cache_capacity:
                        inputs["write_mask"] = inputs["write_mask"][:, :, :cache_capacity, :].copy()
            ...
            # MLState is mutable. Preserve immutable arrays so the ready state
            # can be restored after every completed or cancelled request.
            self.prefix_state_snapshot = [
                tuple(state.read_state(name).copy() for name in ("key_cache", "value_cache"))
                for state in self.prefix_states
            ]

    def restore_prefix_states(self):
        """Prepare the next request outside its first-PCM critical path."""
        prefix_blocks = self.short_blocks or self.blocks
        restored = []
        for block, (keys, values) in zip(prefix_blocks, self.prefix_state_snapshot, strict=True):
    ...
    def zero_cache(model):
        ...
        shape = next(
            i for i in model.source_spec.description.input if i.name == "keys"
    ...
    def verify_windows(self, chunks):
        ...
        for frame, chunk in enumerate(chunks):
            ...
            history[0, :, index] = np.stack(
                [table[code] for table, code in zip(lookups, chunk["codes"], strict=True)]
            ).sum(0)
            expected = self.decoder.predict({"latent": history})["pcm"][
                0, index * 1920 : (index + 1) * 1920
            ]
            width, offset = decoder_window(frame)
            actual = self.decoder_windows[width].predict({"latent": history[:, :, :width].copy()})[
                "pcm"
            ][0, offset * 1920 : (offset + 1) * 1920]
            if frame == 0 and self.startup_decoder is not None:
                actual = self.startup_decoder.predict({"latent": history[:, :, :1].copy()})["pcm"][
                    0
                ]
        ...
        return {
            "scope": "Windowed vs original Core ML decoder on every generated frame; not FP32 source or listening validation",
            "frames": len(errors),
    ...
    def chunks_for_text(self, text, max_frames=64, *, incremental=False):
        ...
        try:
            if incremental:
                ...
                text_source = TextParts(
                    self.frontends[0],
                    text,
                    lambda value: capture_prompt(
                        self.frontends[0], value, self.instruction, self.speaker, self.capacity
                    ),
                )
                arrays = text_source.initial
            else:
                arrays = capture_prompt(
                    self.frontends[0], text, self.instruction, self.speaker, self.capacity
                )
                ...
                for payload, metadata in stream:
                    yield (
                        ...
                        dict(
                            ...
                            text_characters_received=len(text_source.buffer)
                            if text_source
                            else len(text),
                        ),
                    )
    ...
    def chunks(self, max_frames=64):
        ...
        if not self.prefix_lock.acquire(blocking=False):
            raise RuntimeError(
                "Prefix state is already in use; independent sessions need independent state"
            )
        try:
            ...
            if hasattr(self, "prefix_state_snapshot"):
                self.restore_prefix_states()
    ...
    def _chunks(self, max_frames=64):
        ...
        caches = (
            list(self.prefix_states)
            if self.prefix_states is not None
            else [None for _ in self.blocks]
        )
        ...
        def advance(hidden):
            ...
            if self.long_blocks and position == 128:
                caches[0] = tuple(
                    caches[0].read_state(name).copy() for name in ["key_cache", "value_cache"]
                )
                long_active = True
            if short_active and position == 16:
                for index, state in enumerate(caches):
                    caches[index] = tuple(
                        state.read_state(name).copy() for name in ["key_cache", "value_cache"]
                    )
                short_active = False
            active_blocks = (
                self.long_blocks
                if long_active
                else self.short_blocks
                if short_active
                else self.blocks
            )
            ...
            for index, block in enumerate(active_blocks):
                ...
                if not np.isfinite(hidden).all():
                    raise RuntimeError(
                        f"Non-finite talker hidden state at position {position}, block {index}"
                    )
            position += 1
        ...
        for frame in range(max_frames):
            ...
            scores = penalize_repetitions(
                logits[0, 0], semantic_history, arrays["repetition_penalty"]
            )
            ...
            if self.fused_predictor:
                ...
                if (
                    ...
                    or np.any(residual != np.floor(residual))
                    or np.any(residual < 0)
                    or np.any(residual >= 2048)
                ):
                ...
                for step in range(16):
                    ...
                    if step == 0:
                        ...
                        if not np.isfinite(result["logits"][0, step - 1]).all():
                            raise RuntimeError(
                                f"Non-finite predictor logits at frame {frame}, step {step}"
                            )
            ...
            latent_history[0, :, decoder_index] = np.stack(
                [table[code] for table, code in zip(lookups, codes, strict=True)]
            ).sum(0)
            use_tail = frame >= 31 and self.tail_decoder is not None
            decoder = (
                self.tail_decoder
                if use_tail
                else self.first_decoder
                if frame < self.first_decoder_frames
                else self.decoder
            )
            ...
            if self.decoder_windows:
                ...
                shape = tuple(
                    next(
                        x for x in decoder.source_spec.description.input if x.name == "latent"
                    ).type.multiArrayType.shape
                )
            ...
            if self.history_decoder:
                sample = dict(
                    controls[2][frame],
                    latent=latent_history[:, :, decoder_index : decoder_index + 1].copy(),
                    **decoder_state,
                )
                decoded = self.decoder.predict(sample)
                pcm = decoded["pcm"][0]
                decoder_state = {
                    name: decoded["out_" + name[3:]] for name in self.decoder_state_shapes
                }
                ...
                pcm = self.startup_decoder.predict({"latent": latent_history[:, :, :1].copy()})[
                    "pcm"
                ][0]
                ...
                pcm = (
                    decoded[0, :1920]
                    if use_tail
                    else decoded[0, decoder_index * 1920 : (decoder_index + 1) * 1920]
                )
            ...
            hidden += (
                text_source.next_hidden()
                if text_source is not None
                else trailing[:, frame : frame + 1]
                if frame < trailing.shape[1]
                else arrays["tts_pad_embed"]
            )
...
def main():
    ...
    parser.add_argument(
        "--instruction",
        default="A natural American English female voice. Start with sincere empathy, pause after charge, then sound confident and reassuring.",
    )
    parser.add_argument(
        "--compiled-dir",
        type=Path,
        help="Content-addressed compiled model cache; always rechecks compute plans, never caches audio",
    )
    parser.add_argument(
        "--predictor-package",
        type=Path,
        help="Explicit candidate predictor override; still requires strict ANE admission",
    )
    parser.add_argument(
        "--prefill-packages",
        type=Path,
    ...
    parser.add_argument(
        ...
        type=int,
        default=0,
        help="Additional warm first-chunk trials on the same prepared prompt, not client latency",
    )
    parser.add_argument(
        "--first-decoder-package",
        type=Path,
        help="Short decoder used while the complete history fits its input window",
    )
    parser.add_argument(
        "--speaker", help="Required for CustomVoice; does not enable emotion instructions"
    )
    ...
    parser.add_argument(
        "--experimental-history-decoder",
        action="store_true",
        help="Explicit-state decoder experiment; not source/audio quality validated",
    )
    parser.add_argument(
        "--experimental-prefix-state",
        action="store_true",
        help="Reuse invariant 9-token CustomVoice KV prefix, not generated audio; serial experimental sessions only",
    )
    parser.add_argument(
        "--short-talker-package",
        type=Path,
        help="Experimental 16-position initial KV cache with migration to the full talker",
    )
    parser.add_argument(
        "--text-projection-package",
        type=Path,
        help="Experimental ANE learned text projection; tokenizer and lookups remain host-side",
    )
    parser.add_argument(
        "--long-talker-package",
        type=Path,
        help="Experimental stateful 512-position talker, migrated to only after position 127",
    )
    parser.add_argument(
        "--frontend-assets",
        type=Path,
        help="Minimal exported tokenizer/lookup assets; avoids loading the upstream checkpoint",
    )
    parser.add_argument(
        "--verify-prefix-state",
        action="store_true",
        help="Compare every PCM chunk and code against fresh sequential prefill; extra synthesis, outside timing",
    )
    parser.add_argument(
        "--tail-decoder-package",
        type=Path,
        help="Experimental exact-tail decoder used once the 32-frame history is full",
    )
    parser.add_argument(
        "--decoder-windows",
        type=Path,
        help="Four-output-frame tail graphs for input widths 8,12,...32",
    )
    parser.add_argument(
        "--startup-decoder-package",
        type=Path,
        help="One-frame decoder used only at frame zero, never for continuation",
    )
    parser.add_argument(
        "--capture-predictor-inputs",
        type=Path,
        help="Save real predictor inputs for experimental activation calibration",
    )
    parser.add_argument("--verify-decoder-windows", action="store_true")
    parser.add_argument(
        "--max-frames", type=int, default=64, choices=range(1, 503), metavar="1..502"
    )
    ...
    voice = VoiceStream(
        ...
        args.instruction,
        args.compiled_dir,
        args.predictor_package,
        args.prefill_packages,
        args.first_decoder_package,
        args.speaker,
        args.model_prefix,
        args.block_count,
        args.decoder_package,
        args.tail_decoder_package,
        args.decoder_windows,
        args.startup_decoder_package,
        args.experimental_history_decoder,
        args.experimental_prefix_state,
        args.short_talker_package,
    ...
    report = {
        "text": args.text,
        "instruction": None if args.speaker else args.instruction,
        ...
        "short_talker_package": str(args.short_talker_package)
        if args.short_talker_package
        else None,
        "text_projection_package": str(args.text_projection_package)
        if args.text_projection_package
        else None,
        ...
        "first_decoder_package": str(args.first_decoder_package.resolve())
        if args.first_decoder_package
        else None,
        "tail_decoder_package": str(args.tail_decoder_package.resolve())
        if args.tail_decoder_package
        else None,
        "decoder_windows": str(args.decoder_windows.resolve()) if args.decoder_windows else None,
        "startup_decoder_package": str(args.startup_decoder_package.resolve())
        if args.startup_decoder_package
        else None,
        "predictor_package": str(args.predictor_package.resolve())
    ...
    if args.verify_prefix_state:
        ...
        try:
            ...
            equal = (
                len(reference) == len(parity_payloads)
                and all(
                    payload == expected and metadata["codes"] == previous["codes"]
                    for (payload, metadata), expected, previous in zip(
                        reference, parity_payloads, chunks, strict=True
                    )
                )
        ...
        parity = {
            "scope": "Full generated PCM/codes vs fresh sequential ANE prefill, not source-model quality",
            "frames": len(chunks),
            "reference_frames": len(reference),
            "exact_match": equal,
            "audio_cache_used": False,
        }
        args.output.with_suffix(".prefix-parity.json").write_text(
    ...
    if args.capture_predictor_inputs:
        ...
        np.savez(
            args.capture_predictor_inputs,
            **{
                name: np.stack([sample[name] for sample in voice.predictor_samples])
    ...
    if args.verify_decoder_windows:
        ...
        args.output.with_suffix(".decoder-parity.json").write_text(
    ...
    if args.first_chunk_trials:
        ...
        for trial in range(args.first_chunk_trials + 5):
            ...
            try:
                ...
                generator.close()
        benchmark = {
            "scope": "warm prepared fixed prompt to first PCM; excludes preparation and transport",
            "samples": len(timings),
            "warmup_runs": 5,
            "audio_cache_used": False,
            ...
            "stages_ms": {
                name: {
                    "p50": float(np.percentile([t[name] for t in timings], 50)),
