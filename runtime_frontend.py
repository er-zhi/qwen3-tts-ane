...
class ArrayEmbedding:
    def __init__(self, path):
        ...
        if correction.exists():
            ...
            self.flat_index=saved['flat_index']; self.value=saved['value']
        else:
            self.flat_index=np.empty(0,np.int64); self.value=np.empty(0,np.float32)

    def __call__(self, indices):
        index=np.asarray(indices)
        output=np.asarray(self.array[index],dtype=np.float32)
        rows=index.reshape(-1); values=output.reshape(-1,self.array.shape[1])
        for position,row in enumerate(rows):
            start=int(row)*self.array.shape[1]; stop=start+self.array.shape[1]
            left=np.searchsorted(self.flat_index,start); right=np.searchsorted(self.flat_index,stop)
            if right>left:
...
class NumpyANETextProjection:
    def __init__(self,model,channels,width,output_channels):
        self.model,self.channels,self.width,self.output_channels=model,channels,width,output_channels
...
class Talker:
    def __init__(self,assets,metadata,text_projection):
        ...
        self.predictor_embeddings = [ArrayEmbedding(assets/f'predictor_embedding_{i:02}.npy')
...
class RuntimeFrontend:
    def __init__(self,assets,text_projection):
        ...
        self.tokenizer=AutoTokenizer.from_pretrained(str(self.assets/'tokenizer'),
    ...
    def _tokenize_texts(self,texts):
        return [self.tokenizer(text,return_tensors='np',padding=True)['input_ids'] for text in texts]

    def project_ids(self,ids):
        return self.model.talker.text_projection(self.model.talker.get_text_embeddings()(np.asarray([ids])))
    ...
    def capture_prompt(self,text,speaker):
        ...
        project=lambda values: talker.text_projection(talker.get_text_embeddings()(values))
        special=np.asarray([[self.metadata[name] for name in
            ('tts_bos_token_id','tts_eos_token_id','tts_pad_token_id')]])
        bos,eos,pad=np.split(project(special),3,axis=1)
        codec_ids=self.metadata['codec_prefix_ids']+[self.metadata['speaker_id'],
            self.metadata['codec_pad_id'],self.metadata['codec_bos_id']]
        codec=talker.get_input_embeddings()(np.asarray([codec_ids]))
        role=project(ids[:,:3])
        prefix=np.concatenate((np.repeat(pad,codec.shape[1]-2,axis=1),bos),axis=1)+codec[:,:-1]
        initial=np.concatenate((role,prefix,project(ids[:,3:4])+codec[:,-1:]),axis=1)
        trailing=np.concatenate((project(ids[:,4:-5]),eos),axis=1)
        return {'inputs_embeds':initial.copy(),
...
def load_controls(assets,metadata):
    ...
    for group in metadata['control_groups']:
        fields=group['fields']; length=group['length']
        groups.append([{field:arrays[f'{group["name"]}_{field}'][position] for field in fields}
...
def prepare_runtime_assets(assets,text,speaker,text_projection,frontend_sink):
    ...
    lookups=[np.load(assets/f'lookup_{i:02}.npy',mmap_mode='r')
             for i in range(metadata['lookups'])]
    if frontend_sink is not None:
        frontend_sink.append(frontend)
    return arrays,embeddings,lookups,load_controls(assets,metadata),metadata['codec_eos_token_id']
